...
public class MonsterMove : MonoBehaviour
{
    ...
    void MonsterrMove()
    {
        ...
        if (seePlayer)//开启判定，怪物开始追踪player
        {
            ...
            {
                if (!Physics2D.Raycast(transform.position, currDirX, 1.5f,1<<8|1<<7))//1.1 X轴无碰撞体 ，从怪物发射射线，（原点，向量方向，射线长度，碰撞检测层级），TRUE的话有碰撞体，FALSE无碰撞体
                ...
                //实现怪物自动开门的功能
                else if(!Physics2D.Raycast(transform.position, currDirX, 1.5f, 1 << 8)&& //第7层有门，第8层无障碍物
                    (Physics2D.Raycast(transform.position, currDirX, 1.5f, 1 << 7))){

                    RaycastHit2D hitX = Physics2D.Raycast(transform.position, currDirX, 1.5f, 1 << 7);//获取到射线检测到的物体——门
                    restTimer2 += Time.deltaTime;//设定将门销毁的时间，3s，3s之后，门销毁，怪物继续移动，否则返回
                    if (restTimer2 < restTime2)
                        return;
                    if (hitX.collider != null)Destroy(hitX.collider.gameObject);//如果有门，就将门销毁
                    enemypos += new Vector3(1.5f * currDir.x / Mathf.Abs(currDir.x), 0f, 0f);//怪物向X轴移动单位距离，x++
                    restTimer2 = 0;
                    restTimer = 0;
                    Debug.Log("摧毁门");
                ...
                {
                    if (!Physics2D.Raycast(transform.position, currDirY, 1.5f, 1 << 8|1<<7))//1.2.1 Y轴无碰撞体
                    {//无门
            ...
            {
                if (!Physics2D.Raycast(transform.position, currDirY, 1.5f, 1 << 8 | 1 << 7))//2.1 Y轴无碰撞体
                {
                ...
                {
                    if (!Physics2D.Raycast(transform.position, currDirX, 1.5f, 1 << 8 | 1 << 7))//2.2.1 X轴无碰撞体
                    {
